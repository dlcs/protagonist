...
* Would we want `on delete cascade` for `AssetApplicationMetadata`? If we store all possible derivative keys in this the CleanupHandler could use that to do more targeted cleanup.