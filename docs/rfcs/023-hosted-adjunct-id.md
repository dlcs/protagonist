...
This is specific to `iiifLink` of `annotation` and `inline-annotation`. The API user has specified that the
...
for the solution `2` required in both cases, and therefore much more sensible asd a general solution.
...
Therefore, uplifting both the processor and the `id`-rewrite plugin into e.g. a dedicated NuGet package could allow us
...
* Stream the processor output to user response
