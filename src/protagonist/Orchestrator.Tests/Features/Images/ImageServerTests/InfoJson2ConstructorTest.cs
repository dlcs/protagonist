using System.Collections.Generic;
using System.Threading;
using DLCS.Core.Types;
using DLCS.Repository.Assets;
using FakeItEasy;
using IIIF;
using IIIF.Auth.V2;
using IIIF.ImageApi;
using IIIF.ImageApi.V2;
using Microsoft.Extensions.Logging.Abstractions;
using Orchestrator.Assets;
using Orchestrator.Features.Images.ImageServer;
using Orchestrator.Infrastructure.IIIF;
using Orchestrator.Tests.Integration;

namespace Orchestrator.Tests.Features.Images.ImageServerTests;

public class InfoJson2ConstructorTest : InfoJson2Constructor
{
    private static readonly IIIIFAuthBuilder IiifAuthBuilder = A.Fake<IIIIFAuthBuilder>();
    private static readonly IIIFAuth1Builder IiifAuth1Builder = A.Fake<IIIFAuth1Builder>();
    private static readonly ThumbRepository ThumbRepository = A.Fake<ThumbRepository>();

    public InfoJson2ConstructorTest() : base(IiifAuthBuilder, IiifAuth1Builder,new FakeImageServerClient(), ThumbRepository,
        new NullLogger<InfoJson2Constructor>())
    {
    }

    [Theory]
    [InlineData(512)]
    [InlineData(256)]
    public void SetImageTileServiceSizes_UpdatesTileSizeIfNeeded(int tileSize)
    {
        // Arrange
        var imageService = new ImageService2
        {
            Tiles = new List<Tile>{new Tile()
            {
                Height = tileSize,
                Width = tileSize
            }}
        };

        // Act
        SetImageTileServiceSizes(imageService, 500);

        // Assert
        imageService.Tiles[0].Width.Should().Be(256);
        imageService.Tiles[0].Height.Should().Be(256);
    }
    
    [Fact]
    public void SetImageTileServiceSizes_UpdatesTileSizeWhenTilesNull()
    {
        // Arrange
        var imageService = new ImageService2
        {
            Tiles = null
        };

        // Act
        SetImageTileServiceSizes(imageService, 500);

        // Assert
        imageService.Tiles[0].Width.Should().Be(256);
        imageService.Tiles[0].Height.Should().Be(256);
    }
    
    [Fact]
    public void SetImageTileServiceSizes_UpdatesTileSize_WhenMultipleTiles()
    {
        // Arrange
        var imageService = new ImageService2
        {
            Tiles = new List<Tile>{new Tile()
                {
                    Height = 400,
                    Width = 400
                },
                new Tile() {
                    Height = 600,
                    Width = 600
                }}
        };

        // Act
        SetImageTileServiceSizes(imageService, 500);

        // Assert
        imageService.Tiles[0].Width.Should().Be(256);
        imageService.Tiles[0].Height.Should().Be(256);
        imageService.Tiles.Count.Should().Be(1);
    }
    
    [Fact]
    public void SetImageTileServiceSizes_DoesNotUpdateTileSize_WhenMultipleTilesSmaller()
    {
        // Arrange
        var imageService = new ImageService2
        {
            Tiles = new List<Tile>{new Tile()
                {
                    Height = 400,
                    Width = 400
                },
                new Tile() {
                    Height = 300,
                    Width = 300
                }}
        };

        // Act
        SetImageTileServiceSizes(imageService, 500);

        // Assert
        imageService.Tiles[0].Width.Should().Be(400);
        imageService.Tiles[0].Height.Should().Be(400);
        imageService.Tiles.Count.Should().Be(2);
    }
    
    [Fact]
    public void SetImageTileServiceSizes_UpdatesSize()
    {
        // Arrange
        var imageService = new ImageService2
        {
            Sizes = new List<Size> { new Size(256, 256) }
        };

        var newSizes = new List<Size> { new Size(512, 512) };
        

        // Act
        SetImageServiceSizes(imageService, newSizes);

        // Assert
        imageService.Sizes[0].Width.Should().Be(512);
        imageService.Sizes[0].Height.Should().Be(512);
    }
    
    [Fact]
    public void SetImageServiceStubId_UpdatesServiceStubId()
    {
        // Arrange
        var imageService = new ImageService2();
        imageService.Id = "someId";

        var orchestrationImage = new OrchestrationImage
        {
            AssetId = new AssetId(1, 1, "someAsset")
        };

        // Act
        SetImageServiceStubId(imageService, orchestrationImage);

        // Assert
        imageService.Id.Should().Be("v2/1/1/someAsset");
    }
    
    [Fact]
    public void SetImageServiceMaxWidth_UpdatesMaxWidth()
    {
        // Arrange
        var imageService = new ImageService2
        {
            ProfileDescription = new ProfileDescription
            {
                MaxArea = 500,
                MaxHeight = 500,
                MaxWidth = 500
            }
        };

        var orchestrationImage = new OrchestrationImage();
        orchestrationImage.MaxUnauthorised = 250;
        
        // Act
        SetImageServiceMaxWidth(imageService, orchestrationImage);

        // Assert
        imageService.ProfileDescription.MaxArea.Should().BeNull();
        imageService.ProfileDescription.MaxHeight.Should().BeNull();
        imageService.ProfileDescription.MaxWidth.Should().Be(250);
    }
    
    [Fact]
    public async Task SetImageServiceAuthServices_UpdatesAuthService()
    {
        // Arrange
        var imageService = new ImageService2();

        var authService = new AuthProbeService2();

        A.CallTo(() => IiifAuthBuilder.GetAuthServicesForAsset(A<AssetId>._, A<List<string>>._, A<CancellationToken>._))
            .Returns(authService);

        // Act
        await SetImageServiceAuthServices(imageService, new OrchestrationImage(), new CancellationToken());

        // Assert
        imageService.Service.Count.Should().Be(1);
    }
}