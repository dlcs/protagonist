...
public class ImageBatchPatchValidator : AbstractValidator<HydraCollection<DLCS.HydraModel.Image>>
{
    public ImageBatchPatchValidator(IOptions<ApiSettings> apiSettings)
    {
        ...
        RuleForEach(c => c.Members).ChildRules(members =>
        {
            ...
            members.RuleFor(a => a.MaxWidth).Empty().WithMessage("MaxWidth cannot be set in a bulk patching operation");
            members.RuleFor(a => a.OpenFullMax).Empty().WithMessage("OpenFullMax cannot be set in a bulk patching operation");
