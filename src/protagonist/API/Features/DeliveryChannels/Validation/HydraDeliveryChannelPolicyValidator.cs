...
public class HydraDeliveryChannelPolicyValidator : AbstractValidator<DLCS.HydraModel.DeliveryChannelPolicy>
{
    ...
    public HydraDeliveryChannelPolicyValidator(DeliveryChannelPolicyDataValidator policyDataValidator)
    {
        ...
        RuleFor(p => p.PolicyData)
            .MustAsync((p, pd, _) => IsValidPolicyData(pd, p.Channel))
            .When(p => !string.IsNullOrEmpty(p.PolicyData))
            .WithMessage(_ => "'policyData' contains bad JSON or invalid data");
        RuleFor(p => p.Modified)
            .Empty().WithMessage(_ => "'policyModified' is generated by the DLCS and cannot be set manually");
        RuleFor(p => p.Created)
            .Empty().WithMessage(_ => "'policyCreated' is generated by the DLCS and cannot be set manually");
    }
